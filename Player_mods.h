...
inline void PlayerBuff(ENetPeer* peer, int x, int y)
{
	...
	if (pinfo->Fishing && !pinfo->TriggerFish && pinfo->FishPosX != 0 && pinfo->FishPosY != 0)
	{
		const auto values = rand() % 300 + 1;
		if (values <= 3)
		{
			pinfo->TriggerFish = true;
			     ...
			     ++currentPeer)
			{
				...
				{
					Player::OnParticleEffect(currentPeer, 36, pinfo->FishPosX, pinfo->FishPosY, 0);
					Player::PlayAudio(currentPeer, "audio/splash.wav", 0);
				}
			}
		}
	}
