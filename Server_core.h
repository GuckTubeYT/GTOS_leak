...
const char* user = "JsOuYekJUT";
const char* pass = "anZM6rcdhR";
const char* database = "JsOuYekJUT";
MYSQL* conn;
MYSQL_RES* res;
MYSQL* conn3;
MYSQL_RES* res3;
bool Connected = false;
int configPort = 17093;
ENetHost* server;
...
inline void SendConsole(const string text, const string type)
{
	...
	if (type == "BAN")
	{
		const auto superrealtext = space2underscore("[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + " " + type + """]: " + text);
		const auto themessage = "start php banmanagersystem.php ```" + superrealtext + "```";
		const auto c = themessage.c_str();
		system(c);
		return;
	}
	if (type == "CHAT" && text.find("@everyone") == string::npos)
	{
		const auto superrealtext = space2underscore(text);
		const auto themessage = "start php system.php ```" + superrealtext + "```";
		const auto c = themessage.c_str();
		system(c);
	}
}
...
inline void ConnectToDatabase()
{
	...
	mysql_ct = 28800;
	mysql_options(conn, MYSQL_OPT_CONNECT_TIMEOUT, &mysql_ct);
