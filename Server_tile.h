...
inline void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
{
	...
	case 32: /*wrench*/
		{
			...
			if (getItemDef(world->items[x + (y * world->width)].foreground).blockType == BlockTypes::SUCKER) /*magplant gaia ut*/
			{
				...
				if (ismag)
				{
					...
					bool Farmable = false;
					if (itemid == 7382 || itemid == 4762 || itemid == 10024 || itemid == 9468 || itemid == 9460 || itemid == 10028) Farmable = true;
	...
	case 6286: case 6288: case 6290: case 656: case 658: case 1006: case 8878: case 1452:
		{
	...
	case 6856: case 6858: case 6860: case 6862: case 9266: /*subtokens*/
		{
			...
			if (x == static_cast<PlayerInfo*>(peer->data)->x / 32 && y == static_cast<PlayerInfo*>(peer->data)->y / 32)
			{
				...
				{
					stringstream ss;
					ss << "UPDATE PlayerDatabase SET subtype = '" + Sub_Type + "', subdate = '" + to_string(Sub_Days) + "' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
					auto query = ss.str();
					...
					{
						stringstream sse;
						sse << "SELECT * FROM PlayerState WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
						auto cs = sse.str();
						auto strings = cs.c_str();
			...
			if (tile == 528) /*luckyclover*/
			{
				if (x == static_cast<PlayerInfo*>(peer->data)->x / 32 && y == static_cast<PlayerInfo*>(peer->data)->y / 32)
				{
					if (static_cast<PlayerInfo*>(peer->data)->LuckyClover == false)
					{
						...
						Player::OnConsoleMessage(peer, "You're `$luckier `othan before! (`$Lucky! `omod added, `$15 mins`o left)");
						static_cast<PlayerInfo*>(peer->data)->lastClover = 900;
	...
	case 6204: case 6202: case 6200: case 7484: case 7954: /*chest*/
		{
			if (tile == 6204) 
			{
	...
	if (tile == 18)
	{
		...
		{
			if (world->items[x + (y * world->width)].foreground == 4762 || world->items[x + (y * world->width)].foreground == 7382 || world->items[x + (y * world->width)].foreground == 9468 || world->items[x + (y * world->width)].foreground == 10024 || world->items[x + (y * world->width)].foreground == 10028 || world->items[x + (y * world->width)].foreground == 9460)
			{
		...
		{
			if (y < world->height && world->items[x + (y * world->width)].breakLevel + 4 >= brak.breakHits * 4)
			{
				data.packetType = 0x3;
				data.netID = causedBy;
				data.plantingTree = 18;
...
#pragma endregion
					if (static_cast<PlayerInfo*>(peer->data)->xp >= (static_cast<PlayerInfo*>(peer->data)->level * 1500) && static_cast<PlayerInfo*>(peer->data)->level < 125)
					{
						...
						{
							stringstream ss;
							ss << "UPDATE PlayerDatabase SET level = '" + to_string(static_cast<PlayerInfo*>(peer->data)->level) + "', xp = '" + to_string(static_cast<PlayerInfo*>(peer->data)->xp) + "' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
							auto query = ss.str();
							const auto q = query.c_str();
							if (mysql_query(conn, q))
							{
	...
	else
	{
		...
		for (auto i = 0; i < static_cast<PlayerInfo*>(peer->data)->inventory.items.size(); i++)
		{
			if (static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemID == tile)
			{
				if (static_cast<unsigned int>(static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemCount) > 1)
				{
					...
					SaveInventoryWhenBuildingBlock(peer, tile);
				}
				else
				{
		...
		{
			bool Farmable = false;
			if (tile == 7382 || tile == 4762 || tile == 10024 || tile == 9468 || tile == 9460 || tile == 10028) Farmable = true;
