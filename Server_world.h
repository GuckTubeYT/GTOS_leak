...
inline void sendPlayerLeave(ENetPeer* peer, PlayerInfo* player)
{
	...
	{
		stringstream ss;
		ss << "UPDATE PlayerDatabase SET xp = '" + to_string(static_cast<PlayerInfo*>(peer->data)->xp) + "' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
...
inline int DropItem(ENetPeer* peer, int netID, int x, int y, int item, int count, BYTE specialEffect)
{
	...
	if (item < 0) return -1;
	if (x == 0 || y == 0) return -1;
	if (static_cast<PlayerInfo*>(peer->data)->currentWorld == "EXIT") return -1;
	auto blockX = x / 32;
	auto blockY = y / 32;
...
inline void SaveDroppedItem(int fItemid, int fQuantity, ENetPeer* peer, bool& success)
{
	...
	else
	{
		...
		{
			SendConsole("SaveDroppedItem Critical error details: rawName(" + static_cast<PlayerInfo*>(peer->data)->rawName + ")", "ERROR");
			if (!static_cast<PlayerInfo*>(peer->data)->Console) enet_peer_disconnect_now(peer, 0);
		}
		catch (std::runtime_error&)
		{
			SendConsole("SaveDroppedItem Critical error details: name(" + static_cast<PlayerInfo*>(peer->data)->rawName + ")", "ERROR");
			if (!static_cast<PlayerInfo*>(peer->data)->Console) enet_peer_disconnect_now(peer, 0);
		}
		catch (...)
		{
...
inline void send_item(ENetPeer* peer, const int itemid, const int item_amount)
{
	auto success = false;
...
inline void DailyRewardCheck(ENetPeer* peer)
{
	if (static_cast<PlayerInfo*>(peer->data)->lastdailyGems <= GetCurrentTimeInternalSeconds() / 60)
	{
		...
		auto haveGems = 0;
		ifstream getgems("gemdb/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
		getgems >> haveGems;
		getgems.close();
		auto gemsReward = 15000;
		if (static_cast<PlayerInfo*>(peer->data)->Subscriber && static_cast<PlayerInfo*>(peer->data)->SubscribtionEndDay - 1 >= 1) gemsReward += 15000;
		haveGems += gemsReward;
		//Player::OnConsoleMessage(peer, "`^You got `2" + to_string(gemsReward) + " `9Gems `^For joining the server!");
		auto ps2notf = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/science_button.rttex"), "`^You Received `2" + to_string(gemsReward) + " `9Gems `^For joining the server!"), "audio/hub_open.wav"), 0));
		auto packet3s = enet_packet_create(ps2notf.data, ps2notf.len, ENET_PACKET_FLAG_RELIABLE);
		enet_peer_send(peer, 0, packet3s);
		delete ps2notf.data;
		ofstream myfileGems("gemdb/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
		myfileGems << haveGems;
		myfileGems.close();
		if (conn == nullptr)
		{
		...
		if (conn != nullptr)
		{
			...
			if (static_cast<PlayerInfo*>(peer->data)->Subscriber)
			{
				...
				{
					ss << "UPDATE PlayerDatabase SET dailyrewards = '" + to_string(static_cast<PlayerInfo*>(peer->data)->lastdailyGems) + "', subtype = '', subdate = '' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
				}
				else
				{
					ss << "UPDATE PlayerDatabase SET dailyrewards = '" + to_string(static_cast<PlayerInfo*>(peer->data)->lastdailyGems) + "', subdate = '" + to_string(static_cast<PlayerInfo*>(peer->data)->SubscribtionEndDay) + "' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
				}
			}
			else
			{
				ss << "UPDATE PlayerDatabase SET dailyrewards = '" + to_string(static_cast<PlayerInfo*>(peer->data)->lastdailyGems) + "' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
			}
			auto query = ss.str();
			const auto q = query.c_str();
			if (mysql_query(conn, q))
			...
			if (static_cast<PlayerInfo*>(peer->data)->SubscribtionEndDay == 0)
			{
				...
				if (static_cast<PlayerInfo*>(peer->data)->cloth_necklace == 6260)
				{
					...
					basic_stringstream << "UPDATE PlayerDatabase SET ClothNeck = '0' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
					const auto basic_string = basic_stringstream.str();
					const auto strings = basic_string.c_str();
	...
	else
	{
		...
		if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
		{
			...
			if (!iscontains)
			{
				...
				Player::OnAddNotification(peer, "`^You Received the `9Growpedia", "audio/hub_open.wav", "interface/cash_icon_overlay.rttex.rttex");
			}
			auto iscontains333 = false;
			SearchInventoryItem(peer, 10034, 1, iscontains333);
			if (!iscontains333)
			{
				auto success = true;
				SaveItemMoreTimes(10034, 1, peer, success);
				Player::OnAddNotification(peer, "`^You Received the `cMerge Book", "audio/hub_open.wav", "interface/cash_icon_overlay.rttex.rttex");
			}
			/*magplant*/
...
inline void SendGhost(ENetPeer* peer)
{
	if (static_cast<PlayerInfo*>(peer->data)->canWalkInBlocks == false)
	{
		...
		stringstream basic_stringstream;
		basic_stringstream << "UPDATE PlayerState SET ghost = 'true' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
	...
	else
	{
		...
		Player::PlayAudio(peer, "audio/dialog_confirm.wav", 0);
		stringstream basic_stringstream;
		basic_stringstream << "UPDATE PlayerState SET ghost = 'false' WHERE username = '" + static_cast<PlayerInfo*>(peer->data)->rawName + "'";
		const auto cs = basic_stringstream.str();
		const auto string = cs.c_str();
		mysql_query(conn, string);
	}
					...
					for (auto i = 0; i < 20; i++)
					{
						try
						{
							if (my_json["safe"][i]["itemid"] == item_id)
							{
								...
								if (static_cast<PlayerInfo*>(peer->data)->adminLevel == 0) pass = "Hidden";
								while ((pos = name.find("worlds\\_")) != std::string::npos)
								{
									name.replace(pos, 8, "");
								}
